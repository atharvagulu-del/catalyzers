...
import { getLectureRecommendationCandidates } from '@/lib/lectureData';
...
export default function TestResultScreen() {
    ...
    const testId = id; // Map id to testId
    ...
    const [viewerMode, setViewerMode] = useState<'question' | 'solution'>('question');

    // Recommendations
    const [recommendations, setRecommendations] = useState<Recommendation[]>([]);
    const [loadingRecommendations, setLoadingRecommendations] = useState(false);
    // Store navigation params for each lecture ID
    const [lectureParamsMap, setLectureParamsMap] = useState<Record<string, any>>({});
    ...
    useEffect(() => {
        async function fetchRecommendations() {
            ...
            try {
                ...
                if (percentage >= 95) performanceLevel = 'outstanding';
                else if (percentage >= 85) performanceLevel = 'excellent';
                ...
                else if (percentage >= 35) performanceLevel = 'below average';
                else performanceLevel = 'needs improvement';

                // Get available lectures for these chapters with FULL PARAMS
                const exam = test.exam_type || 'JEE';
                const chapterIds = test.chapters || [];

                // Use the new helper that returns full navigation context
                const candidates = getLectureRecommendationCandidates(exam, test.subject, test.class, chapterIds);

                // Create map for quick lookup
                const paramsMap: Record<string, any> = {};
                candidates.forEach(c => {
                    paramsMap[c.resource.id] = c.params;
                });
                setLectureParamsMap(paramsMap);

                // Create list for AI prompt
                const lectureList = candidates.map(c => `- ${c.resource.title} (ID: ${c.resource.id})`).join('\n');
...
Based on this performance, recommend 2-3 specific lectures from the list below that will help them improve. 
ONLY recommend lectures from this list.
...
Return ONLY valid JSON in this exact format:
{
    "recommendations": [
        {
            "chapterTitle": "Topic/Chapter Name",
            "reason": "Brief encouraging reason why this will help",
            "resources": [
                { "title": "Exact Lecture Title", "type": "video", "id": "Exact_ID_From_List" }
            ]
        }
    ]
}`;
        ...
        return (
            <SafeAreaView style={{ flex: 1, backgroundColor: colors.bg }} edges={['top']}>
                ...
                <View style={{ padding: 20 }}>
                    ...
                    <View style={{
                        ...
                        <View style={{ flexDirection: 'row', gap: 12, width: '100%' }}>
                            ...
                            {test.solution_paper && test.solution_paper.length > 0 && (
                                <TouchableOpacity
                                    onPress={() => {
                                        setViewerMode('solution');
                                        setViewerVisible(true);
                                    }}
                                    style={{
                                        flex: 1,
                ...
                <TestPaperViewer
                    ...
                    images={viewerMode === 'question' ? (test.test_paper || []) : (test.solution_paper || [])}
                    testName={viewerMode === 'question' ? test.test_name : `${test.test_name} - Solutions`}
    ...
    const hasSolutions = test.solution_paper && test.solution_paper.length > 0;

    // Performance level
    const getPerformanceLevel = (pct: number) => {
        if (pct >= 95) return { label: 'Outstanding', color: '#10B981', bgColor: '#D1FAE5' };
        if (pct >= 85) return { label: 'Excellent', color: '#10B981', bgColor: '#D1FAE5' };
        if (pct >= 75) return { label: 'Very Good', color: '#3B82F6', bgColor: '#DBEAFE' };
        if (pct >= 65) return { label: 'Good', color: '#6366F1', bgColor: '#E0E7FF' };
        if (pct >= 50) return { label: 'Average', color: '#F59E0B', bgColor: '#FEF3C7' };
        if (pct >= 35) return { label: 'Below Average', color: '#F97316', bgColor: '#FFEDD5' };
        return { label: 'Needs Improvement', color: '#EF4444', bgColor: '#FEE2E2' };
    };
        ...
        <SafeAreaView style={{ flex: 1, backgroundColor: colors.bg }} edges={['top']}>
            ...
            <ScrollView contentContainerStyle={{ padding: 16 }} showsVerticalScrollIndicator={false}>
                ...
                >
                    ...
                    {/* Stats Grid - Removed Rank, Full width Percentile */}
                    <View style={{ flexDirection: 'row', gap: 12 }}>
                        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.1)', borderRadius: 16, padding: 14, alignItems: 'center' }}>
                            <Text style={{ fontSize: 11, color: '#FED7AA', fontWeight: '600', marginBottom: 4 }}>PERCENTILE</Text>
                            <Text style={{ fontSize: 24, fontWeight: '700', color: '#FFFFFF' }}>{myPercentile !== null ? `${myPercentile}%` : '-'}</Text>
                            <View style={{ backgroundColor: 'rgba(255,255,255,0.2)', paddingHorizontal: 12, paddingVertical: 4, borderRadius: 20, marginTop: 6 }}>
                                <Text style={{ fontSize: 11, color: '#FFFFFF', fontWeight: '600' }}>{performance.label}</Text>
                            </View>
                        </View>
                    </View>
                </LinearGradient>
                ...
                <View style={{ flexDirection: 'row', gap: 12, marginBottom: 16 }}>
                    ...
                    <TouchableOpacity
                        ...
                        style={{
                            ...
                            justifyContent: 'center',
                            gap: 8,
                            backgroundColor: hasTestPaper ? colors.cardBg : colors.bg,
                            paddingVertical: 14,
                            borderRadius: 12,
                            borderWidth: 1,
                            borderColor: hasTestPaper ? colors.primary : colors.border,
                        }}
                    >
                        <ImageIcon size={18} color={hasTestPaper ? colors.primary : colors.textSecondary} />
                        <Text style={{ fontSize: 13, fontWeight: '600', color: hasTestPaper ? colors.primary : colors.textSecondary }}>
                    ...
                    <TouchableOpacity
                        ...
                        style={{
                            ...
                            justifyContent: 'center',
                            gap: 8,
                            backgroundColor: hasSolutions ? colors.primary : colors.bg,
                            paddingVertical: 14,
                            borderRadius: 12,
                            borderWidth: hasSolutions ? 0 : 1,
                            borderColor: colors.border
                        }}
                    >
                        <BookOpen size={18} color={hasSolutions ? '#FFFFFF' : colors.textSecondary} />
                        <Text style={{ fontSize: 13, fontWeight: '600', color: hasSolutions ? '#FFFFFF' : colors.textSecondary }}>
                            {hasSolutions ? 'View Solution' : 'No Solution'}
                        </Text>
                    </TouchableOpacity>
                </View>
                                ...
                                <View key={i} style={{ backgroundColor: colors.cardBg, borderRadius: 12, padding: 14, borderWidth: 1, borderColor: colors.border }}>
                                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                                        ...
                                        <View style={{ backgroundColor: colors.primary + '20', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6 }}>
                                            <Text style={{ fontSize: 10, fontWeight: '600', color: colors.primary }}>Suggested</Text>
                                        </View>
                                    </View>
                                    <View style={{ flexDirection: 'row', gap: 8, flexWrap: 'wrap' }}>
                                        {rec.resources.slice(0, 2).map((resource, j) => (
                                            <TouchableOpacity
                                                key={j}
                                                onPress={() => {
                                                    // Navigate to lecture player using PRE-CALCULATED params
                                                    const params = lectureParamsMap[resource.id];
                                                    if (params) {
                                                        router.push({
                                                        ...
                                                        console.warn('No params found for lecture', resource.id);
                                                    }
                                                }}
                                                style={{
                                                    flexDirection: 'row',
